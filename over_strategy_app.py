...
st.set_page_config(page_title="Élő Foci Jelzések", layout="wide")
...
def generate_signals(fixtures):
    ...
    for match in fixtures:
        ...
        goals_home = score.get("fulltime", {}).get("home") or 0
        goals_away = score.get("fulltime", {}).get("away") or 0
        fixture_id = fixture["id"]

        stats = get_stats(fixture_id)
        shots_home, shots_away = extract_shots_on_target(stats)
        total_shots = shots_home + shots_away

        # Félidő 0,5 over: 15 perc alatt 3+ kaput eltaláló lövés, de nincs gól
        ...
        # 0,5 over ++: 25. percben 4+ kaput eltaláló lövés
        ...
        # Még egy gól: 60. percben az egyik csapat 1 góllal vezet
...
if not live_fixtures:
    ...
    signals = generate_signals(live_fixtures)

    if signals:
        for s in signals:
            st.markdown(f"### {s['fixture']}")
            st.markdown(f"**Jelzés:** {s['signal']} (perc: {s['elapsed']})")
            if "shots" in s:
                st.write(f"Kapura lövések összesen: {s['shots']}, Gólok: {s['goals']}")
            if "score" in s:
